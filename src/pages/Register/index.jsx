...
function Register() {
    ...
    const [noticeLastName, setNoticeLastName] = useState("")
    const [getFirstName, setGetFirstName] = useState("")
    const [noticeFirstName, setNoticeFirstName] = useState("")
    ...
    const handleSubmit = () => {
        ...
        if (getLastName.length === 0){
            setNoticeLastName("Vui lòng điền Họ của bạn")
        }
        if (getFirstName.length === 0){
            setNoticeFirstName("Vui lòng điền Tên của bạn")
        }
        if (getEmail.length !== 0 && getPassword.length !== 0 && getLastName.length !== 0 && getFirstName !== 0){
            navigate("/")
        }
    }
    ...
    return (
        <>
            ...
            <div className="container mx-auto mt-10 flex flex-col items-center gap-4">
                ...
                {noticeLastName && <p className="text-sm text-red-500">{noticeLastName}</p>}
                <input className="focus:outline-none border py-2 px-4 sm:w-96 w-80 placeholder:text-sm placeholder:text-gray-600 border-gray-400" type="text" placeholder="Họ" onChange={(e) => setGetLastName(e.target.value)} onClick={() => setNoticeLastName("")}/>
                {noticeFirstName && <p className="text-sm text-red-500">{noticeFirstName}</p>}
                <input className="focus:outline-none border py-2 px-4 sm:w-96 w-80 placeholder:text-sm placeholder:text-gray-600 border-gray-400" type="text" placeholder="Tên" onChange={(e) => setGetFirstName(e.target.value)} onClick={() => setNoticeFirstName("")}/>
                ...
                <button className="block sm:w-96 w-80 py-3 bg-main-brown-700 text-white font-semibold text-sm" onClick={handleSubmit}>Đăng ký</button>
